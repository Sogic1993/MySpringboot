			...
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				if(or.sellerConfirm == null){
					content += "<span>賣家回覆 : (賣家尚未回覆)</span><hr>" +
								"<button class='btnupd btn-sm btn-outline-info'>賣家尚未完成確認</button>";
				} else {
					content += "<span>賣家回覆 : " + or.sellerConfirm + "</span><hr>";
					if(or.status == 0)
						content += "<button class='btnupd btn-sm btn-outline-info' value=" + or.caseID + " onclick='swalbtn(this)'>確認修改</button>";
					else
						content += "<button class='btnupd btn-sm btn-outline-info'>已於「" + or.datetime + "」完成退貨流程</button>";
				}
